...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "knot-knot"`
...
# an instantiation. The template submitted the literal `knot-knot` as its own
...
# is set to `owner/knot-knot` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `knot-knot-2`,
